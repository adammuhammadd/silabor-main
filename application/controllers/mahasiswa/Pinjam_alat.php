...
class Pinjam_alat extends CI_Controller
{
    ...
    {
        $alat_dipinjam = $this->db->query('SELECT tb_alat.*, tb_pinjam.status, tb_bidang_lab.*, tb_alat.jumlah_alat - SUM(COALESCE(CASE WHEN tb_pinjam.status = "Telah dikembalikan" THEN 0
        ...
        $data = array(
            'data' => $alat_dipinjam,
            'page' => 'mahasiswa/pinjam_alat/index',
            'link' => 'mahasiswa/pinjam_alat',
            'script' => 'mahasiswa/pinjam_alat/script'
        );
    ...
    {
        $cek_status = $this->db->select('tb_permohonan_pinjam_alat.*, tb_user.*, tb_bidang_lab.*, tb_prodi.*')
            ->from('tb_permohonan_pinjam_alat')
        ...
        $get_kode_2 = $this->db->select('tb_user.*')
            ->from('tb_user')
            ->where('tb_user.is_level', 'Kepala UPT Lab')
            ->get()
            ->row();
    ...
    {
        $get_kode = $this->db->select('tb_permohonan_pinjam_alat.kode_pinjam')
            ->from('tb_permohonan_pinjam_alat')
            ->get();

        if ($get_kode->num_rows() == 0) {
            return 'PPA-1-' . date("dmy");
        }

        if ($get_kode->num_rows() > 0) {
            $last = $this->db->select('tb_permohonan_pinjam_alat.kode_pinjam')
                ->from('tb_permohonan_pinjam_alat')
                ->order_by('id_permohonan_pinjam_alat', "desc")
                ->limit(1)
                ->get()
                ->row();

            $get_kode = substr($last->kode_pinjam, 4, -7) + 1;

            return 'PPA-' . $get_kode . '-' . date("dmy");
        }
    }

    private function cek_alat_lab($lab)
    {
        return $this->db->query('SELECT tb_alat.*, tb_pinjam.status, tb_bidang_lab.*, tb_alat.jumlah_alat - SUM(COALESCE(CASE WHEN tb_pinjam.status = "Telah dikembalikan" THEN 0
                            WHEN tb_pinjam.status = "Ditolak" then 0
                            else tb_pinjam.jml_alat END, 0)) as jumlah_alat_sisa
                            FROM tb_alat
                            LEFT JOIN tb_pinjam ON tb_pinjam.id_alat=tb_alat.id_alat
                            LEFT JOIN tb_bidang_lab ON tb_bidang_lab.id_bidang_lab=tb_alat.id_bidang_lab
                            WHERE bidang_lab="' . $lab . '"
                            GROUP BY tb_alat.id_alat');
    }
}
